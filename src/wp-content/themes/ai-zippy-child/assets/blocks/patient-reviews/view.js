...
"use strict";
/*!*********************************************************************************!*\
  !*** ./src/wp-content/themes/ai-zippy-child/src/blocks/patient-reviews/view.js ***!
  \*********************************************************************************/
...
function updateReviewOverflowStates($slider) {
  $slider.find(".az-child-reviews__card").each((_, card) => {
    const $card = jquery__WEBPACK_IMPORTED_MODULE_0__(card);
    ...
    $card.toggleClass("az-child-reviews__card--is-overflowing", content.scrollHeight - content.clientHeight > 1);
  });
}
function syncCardMinHeight($slider) {
  let $cards = $slider.find(".slick-slide:not(.slick-cloned) .az-child-reviews__card");
  if (!$cards.length) {
  ...
  let maxHeight = 0;
  $cards.each((_, card) => {
    const height = jquery__WEBPACK_IMPORTED_MODULE_0__(card).outerHeight();
    if (height > maxHeight) {
      maxHeight = height;
    }
  });
  if (maxHeight > 0) {
    $slider.find(".az-child-reviews__card").css("min-height", `${Math.ceil(maxHeight)}px`);
  }
}
...
function updateReviewSliders() {
  jquery__WEBPACK_IMPORTED_MODULE_0__(".js-az-child-reviews-slider").each((_, element) => {
    const $slider = jquery__WEBPACK_IMPORTED_MODULE_0__(element);
...
function bindReviewSliders() {
  updateReviewSliders();
  let resizeTimer;
  jquery__WEBPACK_IMPORTED_MODULE_0__(window).on("resize orientationchange", () => {
